...
#### with Inverse Clayton Copula ####


fit_CARleroux_copula_invclayton <- function(formula,
                                      data = NULL,
                                      trials,
                                      W,
                                      burnin = 5000,
  ...
  for(j in 1:n_sample){
    ...
    temp1 <- binomialmcarupdateINVClaytonRW(Wtriplet = W_triplet,
                                            Wbegfin = W_begfin,
                                            nsites = K,
                                            nvar = J,
                                            phi = phi,
    ...
    temp2 <- INVClaytontau2updateRW(Wtriplet = W_triplet,
                                    Wbegfin = W_begfin,
                                    nsites = K,
                                    nvar = J,
                                    phi = phi,
    ...
    if(!fix_rho)
    {
      temp3 <- INVClaytonrhoupdateRW(Wtriplet = W_triplet,
    ...
    if(!fix_alpha)
    {
      temp4 <- INVClaytonalphaupdateRW(Wtriplet = W_triplet,
                                       Wbegfin = W_begfin,
                                       nsites = K, nvar = J,
                                       phi = phi,
                                       denoffset = den_offset,
  ...
  model_string <- c("Likelihood model - Binomial (logit link function)",
                    "\nRandom effects model - Inverse Clayton Copula Leroux MCAR\n")
