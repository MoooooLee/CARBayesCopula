...
fit_CARleroux <- function(formula,
  ...
  if(ncol(trials)!=J) stop("trials has the wrong number of columns.", call_=FALSE)
  ...
  if(!is.numeric(trials)) stop("the numbers of trials has non-numeric values.", call_=FALSE)
  int_check <- N_all - sum(ceiling(trials)==floor(trials))
  if(int_check > 0) stop("the numbers of trials has non-integer values.", call_=FALSE)
  if(min(trials)<=0) stop("the numbers of trials has zero or negative values.", call_=FALSE)
  failures <- trials - Y
  ...
  # }
  # alpha <- alpha
  # if(!is_numeric(alpha) ) stop("alpha is fixed but is not numeric_", call_=FALSE)
  # if(alpha <= 0) stop("alpha is outside the range of Clayton copula (0, Inf)_", call_=FALSE)
  ...
  for(j in 1:n_sample){
    ...
    # 6_2_1 Sample from Y - data augmentation ---------------------------------
    ...
    # 6_2_8 Sample from Sigma -------------------------------------------------

    Sigma_post_scale <- t(phi) %*% Q %*% phi + prior_Sigma_scale
    Sigma <- riwish(Sigma_post_df, Sigma_post_scale)
    Sigma_inv <- solve(Sigma)


    # 6_2_5 Sample from rho ---------------------------------------------------

    if(!fix_rho)
    {
      temp3 <- mcarrhoupdateRW(Wstar = Wstar,
                               Wstar_val = Wstar_val,
                               K = K, J = J,
                               rho = rho,
                               Q = Q,
      ...
      Q <- temp3[[2]]
      det_Q <- temp3[[3]]
      accept_rho[1] <- accept_rho[1] + temp3[[4]]
      accept_rho[2] <- accept_rho[2] + 1
    }else
    {}


    # 6_3 Calculate the deviance ----------------------------------------------
    ...
    # 6_5 Self tune the acceptance probabilities ------------------------------
    ...
    # 6_6 Print progress to the console ---------------------------------------
  ...
  accept_final <- c(accept_beta,
                    ...
                    accept_rho,
                    accept_Sigma)
  names(accept_final) <- c("beta", "phi", "rho","Sigma")
  ...
  summary_hyper[1:J, 1] <- diag(apply(samples_Sigma, c(2,3),
                                      quantile, c(0.5)))
  summary_hyper[1:J, 2] <- diag(apply(samples_Sigma, c(2,3),
                                      quantile, c(0.025)))
  summary_hyper[1:J, 3] <- diag(apply(samples_Sigma, c(2,3),
                                      quantile, c(0.975)))

  summary_hyper[J+1, 1] <- apply(samples_Sigma, c(2,3), quantile,
                                 c(0.5))[1,2]
  summary_hyper[J+1, 2] <- apply(samples_Sigma, c(2,3),
                                 quantile, c(0.025))[1,2]
  summary_hyper[J+1, 3] <- apply(samples_Sigma, c(2,3),
  ...
  if(!fix_rho)
  {
    summary_hyper[(J+2), 1:3] <- quantile(samples_rho, c(0.5, 0.025, 0.975))
    summary_hyper[(J+2), 4:7] <- c(n_keep, accept_rho,
                                   effectiveSize(samples_rho),
                                   geweke.diag(samples_rho)$z)
  }else
  {
    summary_hyper[(J+2), 1:3] <- c(rho, rho, rho)
  ...
  rownames(summary_results)[((J*p)+1): nrow(summary_results)] <- c(
    paste(rep("Sigma",J+1), c(1,2,1), c(1,2,2),
          sep=""),
    "rho")
