...
fit_CARleroux_copula_gumbel <- function(formula,
                                         data = NULL,
                                         trials,
                                         W,
                                         burnin = 5000,
  ...
  if(alpha <= 0) stop("alpha is outside the range of Gumbel copula (1, Inf)_", call.=FALSE)
  ...
  for(j in 1:n_sample){
    ...
    temp1 <- binomialmcarupdateGumbelRW(Wtriplet = W_triplet,
                                        Wbegfin = W_begfin,
                                        nsites = K,
                                        nvar = J,
                                        phi = phi,
    ...
    temp2 <- Gumbeltau2updateRW(Wtriplet = W_triplet,
                                Wbegfin = W_begfin,
                                nsites = K,
                                nvar = J,
                                phi = phi,
    ...
    if(!fix_rho)
    {
      temp3 <- GumbelrhoupdateRW(Wtriplet = W_triplet,
    ...
    if(!fix_alpha)
    {
      temp4 <- GumbelalphaupdateRW(Wtriplet = W_triplet,
                                   Wbegfin = W_begfin,
                                   nsites = K, nvar = J,
                                   phi = phi,
                                   denoffset = den_offset,
  ...
  model_string <- c("Likelihood model - Binomial (logit link function)",
                    "\nRandom effects model - Gumbel Copula Leroux MCAR\n")
