...
fit_CARleroux_copula_gaussian <- function(formula,
                                       data = NULL,
                                       trials,
                                       W,
                                       burnin = 5000,
  ...
  if(rho<0 ) stop("rho is outside the range [0, 1].", call_=FALSE)
  if(rho>1 ) stop("rho is outside the range [0, 1].", call_=FALSE)
  ...
  if(alpha <= 0) stop("alpha is outside the range of Clayton copula (0, Inf)_", call.=FALSE)
  ...
  for(j in 1:n_sample){
    ...
    temp1 <- binomialmcarupdateGaussianRW(Wtriplet = W_triplet,
                                          Wbegfin = W_begfin,
                                          nsites = K,
                                          nvar = J,
                                          phi = phi,
    ...
    temp2 <- Gaussiantau2updateRW(Wtriplet = W_triplet,
                                  Wbegfin = W_begfin,
                                  nsites = K,
                                  nvar = J,
                                  phi = phi,
    ...
    if(!fix_rho)
    {
      temp3 <- GaussianrhoupdateRW(Wtriplet = W_triplet,
    ...
    if(!fix_alpha)
    {
      temp4 <- GaussianalphaupdateRW(Wtriplet = W_triplet,
                                     Wbegfin = W_begfin,
                                     nsites = K, nvar = J,
                                     phi = phi,
                                     denoffset = den_offset,
  ...
  model_string <- c("Likelihood model - Binomial (logit link function)",
                    "\nRandom effects model - Gaussian Copula Leroux MCAR\n")
