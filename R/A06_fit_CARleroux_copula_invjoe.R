...
#### with Inverse Joe Copula ####


fit_CARleroux_copula_invjoe <- function(formula,
                                  data = NULL,
                                  trials,
                                  W,
                                  burnin = 5000,
  ...
  if(alpha <= 1) stop("alpha is outside the range of Joe copula (1, Inf).", call.=FALSE)
  ...
  for(j in 1:n_sample){
    ...
    temp1 <- binomialmcarupdateINVJoeRW(Wtriplet = W_triplet,
                                        Wbegfin = W_begfin,
                                        nsites = K,
                                        nvar = J,
                                        phi = phi,
    ...
    temp2 <- INVJoetau2updateRW(Wtriplet = W_triplet,
                                Wbegfin = W_begfin,
                                nsites = K,
                                nvar = J,
                                phi = phi,
    ...
    if(!fix_rho)
    {
      temp3 <- INVJoerhoupdateRW(Wtriplet = W_triplet,
    ...
    if(!fix_alpha)
    {
      temp4 <- INVJoealphaupdateRW(Wtriplet = W_triplet,
                                   Wbegfin = W_begfin,
                                   nsites = K, nvar = J,
                                   phi = phi,
                                   denoffset = den_offset,
  ...
  model_string <- c("Likelihood model - Binomial (logit link function)",
                    "\nRandom effects model - Inverse Joe Copula Leroux MCAR\n")
