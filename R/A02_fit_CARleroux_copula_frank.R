...
fit_CARleroux_copula_frank <- function(formula,
                                    data = NULL,
                                    trials,
                                    W,
                                    burnin = 5000,
  ...
  if(alpha == 0) stop("alpha is outside the range of Frank copula (positive case) (0,Inf).", call.=FALSE)
  ...
  for(j in 1:n_sample){
    ...
    temp1 <- binomialmcarupdateFrankRW(Wtriplet = W_triplet,
                                       Wbegfin = W_begfin,
                                       nsites = K,
                                       nvar = J,
                                       phi = phi,
    ...
    temp2 <- Franktau2updateRW(Wtriplet = W_triplet,
                               Wbegfin = W_begfin,
                               nsites = K,
                               nvar = J,
                               phi = phi,
    ...
    if(!fix_rho)
    {
      temp3 <- FrankrhoupdateRW(Wtriplet = W_triplet,
    ...
    if(!fix_alpha)
    {
      temp4 <- FrankalphaupdateRW(Wtriplet = W_triplet,
                                  Wbegfin = W_begfin,
                                  nsites = K, nvar = J,
                                  phi = phi,
                                  denoffset = den_offset,
  ...
  model_string <- c("Likelihood model - Binomial (logit link function)",
                    "\nRandom effects model - Frank Copula Leroux MCAR\n")
