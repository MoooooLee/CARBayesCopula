...
fit_CARleroux_copula_joe <- function(formula,
                                             data = NULL,
                                             trials,
                                             W,
                                             burnin = 5000,
  ...
  for(j in 1:n_sample){
    ...
    temp1 <- binomialmcarupdateJoeRW(Wtriplet = W_triplet,
                                     Wbegfin = W_begfin,
                                     nsites = K,
                                     nvar = J,
                                     phi = phi,
    ...
    temp2 <- Joetau2updateRW(Wtriplet = W_triplet,
                             Wbegfin = W_begfin,
                             nsites = K,
                             nvar = J,
                             phi = phi,
    ...
    if(!fix_rho)
    {
      temp3 <- JoerhoupdateRW(Wtriplet = W_triplet,
    ...
    if(!fix_alpha)
    {
      temp4 <- JoealphaupdateRW(Wtriplet = W_triplet,
                                Wbegfin = W_begfin,
                                nsites = K, nvar = J,
                                phi = phi,
                                denoffset = den_offset,
  ...
  model_string <- c("Likelihood model - Binomial (logit link function)",
                    "\nRandom effects model - Joe Copula Leroux MCAR\n")
