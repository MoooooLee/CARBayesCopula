#### This files has a list of common functions that needed for build up functions:

# 1 List of common functions ----------------------------------------------

# common_acceptrates1 - update proposal variance for a MH step based on having no max limit on the proposal var.
# common_acceptrates2 - update proposal variance for a MH step based on having  a max limit on the proposal var.
# common_betablock - Create the blocking structure for beta.
# common_betatransform - back transform the regression parameters to the original scale.
# common_burnin_nsample_thin_check - check the burn-in, n.sample, thin arugments.
# common_frame - check the frame argument.
# common_modelfit - compute the model fit criteria.s
# common_prior_beta_check - Check the prior entered for beta.
# common_prior_varmat_check - check the prior entered for variance matrix parameters.
# common_verbose - check the verbose argument.
# common_Wcheckformat - check the W matrix.

# 2 Code of Functions -----------------------------------------------------

#### Acceptance rates - no maximum limit on the proposal sd
common_acceptrates1 <- function(accept, sd, min, max){
...
common_acceptrates2 <- function(accept, sd, min, max, sd.max){
  #### Update the proposal standard deviations
  rate <- 100 * accept[1] / accept[2]
...
common_betablock <- function(p, blocksize.beta=NULL){
...
common_betatransform <- function(samples.beta, X.indicator, X.mean, X.sd, p, localised){
...
common_burnin_nsample_thin_check <- function(burnin, n.sample, thin){
...
common_frame <- function(formula, data, family){
...
common_modelfit <- function(samples.loglike, deviance.fitted){
  ...
  #### Model fit criteria
  modelfit <- c(DIC, p.d, WAIC, p.w, LMPL, loglike, LOOIC, elpd)
...
common_prior_beta_check <- function(prior.mean.beta, prior.var.beta, p){
  ## Checks
...
common_prior_var_check <- function(prior.var){
  ## Checks
...
common_prior_varmat_check <- function(prior.varmat, J){
...
common_Wcheckformat <- function(W){
