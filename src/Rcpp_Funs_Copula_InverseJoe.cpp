...
// [[Rcpp::export]]
List binomialmcarupdateINVJoeRW(NumericMatrix Wtriplet, NumericMatrix Wbegfin,
  ...
  NumericVector INVCDF_current(nvar), LPDF_current(nvar);
  NumericVector INVCDF_prop(nvar), LPDF_prop(nvar);
  ...
  for(int j = 0; j<nsites; j++)
  {
    ...
    oldpriorbit = JoeDensity2(INVCDF_current, alpha) + sum(LPDF_current);
    newpriorbit = JoeDensity2(INVCDF_prop, alpha) + sum(LPDF_prop);
...
// [[Rcpp::export]]
List INVJoetau2updateRW(NumericMatrix Wtriplet, NumericMatrix Wbegfin,
                     const int nsites, const int nvar,
                     NumericMatrix phi,
  ...
  for(int j=0; j<nsites; j++)
  {
    ...
    oldlikebit[j] = JoeDensity2(INVCDF_current, alpha) + sum(LPDF_current);
    newlikebit[j] = JoeDensity2(INVCDF_prop, alpha) + sum(LPDF_prop);
...
// [[Rcpp::export]]
List INVJoerhoupdateRW(NumericMatrix Wtriplet, NumericVector Wtripletsum,
...
// [[Rcpp::export]]
List INVJoealphaupdateRW(NumericMatrix Wtriplet, NumericMatrix Wbegfin,
                      const int nsites, const int nvar,
                      NumericMatrix phi,
                      NumericVector denoffset,
  ...
  for(int j=0; j<nsites; j++)
  {
    ...
    oldlikebit[j] = JoeDensity2(INVCDF_current, alpha) + sum(LPDF_current);
    newlikebit[j] = JoeDensity2(INVCDF_current, propalpha) + sum(LPDF_current);
  }
