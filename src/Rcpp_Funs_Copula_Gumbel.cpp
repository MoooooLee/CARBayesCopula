...
// [[Rcpp::export]]
List binomialmcarupdateGumbelRW(NumericMatrix Wtriplet, NumericMatrix Wbegfin,
  ...
  NumericVector LCDF_prop(nvar), LPDF_prop(nvar);
  ...
  for(int j = 0; j<nsites; j++)
  {
    ...
    oldpriorbit = GumbelDensity(LCDF_current, alpha) + sum(LPDF_current);
    newpriorbit = GumbelDensity(LCDF_prop, alpha) + sum(LPDF_prop);
...
// [[Rcpp::export]]
List Gumbeltau2updateRW(NumericMatrix Wtriplet, NumericMatrix Wbegfin,
                         const int nsites, const int nvar,
                         NumericMatrix phi,
  ...
  for(int j=0; j<nsites; j++)
  {
    ...
    newlikebit[j] = GumbelDensity(LCDF_prop, alpha) + sum(LPDF_prop);
...
// [[Rcpp::export]]
List GumbelrhoupdateRW(NumericMatrix Wtriplet, NumericVector Wtripletsum,
...
// [[Rcpp::export]]
List GumbelalphaupdateRW(NumericMatrix Wtriplet, NumericMatrix Wbegfin,
                          const int nsites, const int nvar,
                          NumericMatrix phi,
                          NumericVector denoffset,
