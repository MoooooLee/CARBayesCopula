...
// [[Rcpp::export]]
double ClaytonDensity(NumericVector LCDF,
                      NumericVector CDF,
                      double alpha,
                      double nvar){
  double logdens;
...
// [[Rcpp::export]]
List binomialmcarupdateClaytonRW(NumericMatrix Wtriplet, NumericMatrix Wbegfin,
  ...
  for(int j = 0; j<nsites; j++)
  {
    ...
    oldpriorbit = ClaytonDensity(LCDF_current, CDF_current, alpha, nvar) + sum(LPDF_current);
    newpriorbit = ClaytonDensity(LCDF_prop, CDF_prop, alpha, nvar) + sum(LPDF_prop);
...
// [[Rcpp::export]]
List Claytontau2updateRW(NumericMatrix Wtriplet, NumericMatrix Wbegfin,
                         const int nsites, const int nvar,
                         NumericMatrix phi,
  ...
  for(int j=0; j<nsites; j++)
  {
    ...
    newlikebit[j] = ClaytonDensity(LCDF_prop, CDF_prop, alpha, nvar) + sum(LPDF_prop);
...
// [[Rcpp::export]]
List ClaytonrhoupdateRW(NumericMatrix Wtriplet, NumericVector Wtripletsum,
...
// [[Rcpp::export]]
List ClaytonalphaupdateRW(NumericMatrix Wtriplet, NumericMatrix Wbegfin,
                          const int nsites, const int nvar,
                          NumericMatrix phi,
                          NumericVector denoffset,
