---
title: "Bivariate Bayesian Hierarchical Model with Copula"
subtitle: "Vignette of `CARBayesCopula` version 0.3.0"
author: Mu
date: today
...
While missing, `NA` values are allowed in the response variables $(\mathbf{Y}_A, \mathbf{Y}_B)$, missing values in covariates, offsets or trial numbers are not allowed. Trial numbers should be positive integers.
...
Copulas help us to decompose the joint distribution of two random variables into the marginal distributions and the copula function, which can be defined by Sklar's theorem clearly [@sklar1973random].
$$
\begin{align}
...
\end{align}
$$ 
where $F_{A,B}(x,y)$ is the cumulative probability function of the joint distribution of random variables $A$ and $B$, $F_A(x)$ and $F_B(y)$ are the cumulative probability function of the marginal distributions for each of them, and $C_{\alpha}$ is the copula function with parameter $\alpha$, where the input of the copula function is the cumulative probability $u$ and $v$ of the marginal distributions.
...
There are many copula functions available in the literature, and each of them has its own characteristics. Here we only consider some of the copula functions that are widely used in the literature [@genest1993statistical; @nelsen2006introduction]. The copula functions that are available in the package are as follows:
...
# Model Implementation in the Package
 ...
 This example of model implementation is based on the dataset `aedc2021_sa3` and the neighborhood matrix `aedc_W_sa3`, both available in the package. The dataset `aedc2021_sa3` contains the response variables `health` and `social`, `emotional`, `language`, `communication` which are five difference domains of measure of children development that collected by the Australian Early Development Census (AEDC) [@AEDC_Data]. The covariates contain four difference Social-Economic Index of Area (SEIFA) in Australia for each areal unit [@abs_2021_seifa]. The neighborhood matrix `aedc_W_sa3` is a binary matrix that defines the neighborhood structure of the areal units, which is based on the spatial coordinates of the Australia Statistical Geography Standard (ASGS) 2021 SA3 regions [@ASGS2021]. The dataset also contains the spatial coordinates of the areal units, which can be used to visualize the spatial distribution of the variables on maps.

In details, the dataset `aedc2021_sa3` contains the following variables:

  - *SA3_NAME21*, *SA4_NAME21*, *GCC_NAME21*, *STE_NAME21*: These variables represent the names of the geographical units under the Australian Statistical Geography Standard (ASGS) 2021 at different levels.
    + *SA3_NAME21*: The names of the Statistical Area level 3 (SA3) regions.
    + *SA4_NAME21*: The names of the Statistical Area level 4 (SA4) regions.
    + *GCC_NAME21*: The names of the Greater Capital City (GCC) regions.
    + *STE_NAME21*: The names of the State and Territory (STE) regions.
    
  - *IRSD*, *IRSAD* *IER*, *IEO*: These variables represent the Socio-Economic Indexes for Areas (SEIFA) in Australia. Since the original SEIFA values are only available for census years (2006, 2011, 2016, and 2021) under different geographical standards, the values in this dataset have been aligned to the ASGS 2021 version using correspondence files. Additionally, the kernel interpolation method was applied to align the SEIFA values with the Australian Early Development Census (AEDC) years (2009, 2012, 2015, 2018, and 2021).
    + *IRSD*: The Index of Relative Socio-economic Disadvantage. Higher values indicate lower levels of disadvantage.
    + *IRSAD*: The Index of Relative Socio-economic Advantage and Disadvantage. Higher values indicate greater advantage, while lower values indicate greater disadvantage.
    + *IER*: The Index of Relative Socio-economic Advantage and Disadvantage. Higher values indicate greater advantage, while lower values indicate greater disadvantage.
    + *IEO*: The Index of Economic Resources. Higher values represent greater access to economic resources.
    
  - *URP*: The Usual Resident Population of the SA3 regions.
  
  - *domain*: The AEDC domain used to measure childhood developmental vulnerability. This variable can take one of the following values:
    + *health*: The Physical Health and Wellbeing domain.
    + *social*: The Social Competence domain.
    + *emotional*: The Emotional Maturity domain.
    + *language*: The Language and Cognitive Skills domain.
    + *communication*: The Communication Skills and General Knowledge domain.
    
  - *vulnerable*: The number of children in the given SA3 area who are categorized as vulnerable in the specified domain.
  
  - *N*: The total number of children in the sample for the given domain in that area.
  
  - *geometry*: The spatial coordinates of the SA3 regions, which are used to identify neighborhood structures for spatial modeling and visualize the spatial distribution of variables on maps.
...
For this example, we also need the packages `tidyverse` for data analysis and visualization, `spdep` for spatial dependence analysis, `flextable` for table formatting, and`ggpubr` for advanced visualization.
...
fter fitting the Generalized Linear Model (GLM) for the response variables social and language, the residuals and their corresponding pseudo-observations can be calculated. The pseudo-observations of the residuals result from transforming the residuals to uniform marginals, which allows for the estimation of the empirical copula of the bivariate residuals.

The empirical copula provides a non-parametric representation of the dependence structure between the two variables, independent of their marginal distributions. To explore this dependence visually, the pseudo-observations of the residuals can be plotted in a scatter plot, where the x-axis and y-axis represent the transformed residuals. This visualization helps identify potential non-linear or asymmetric relationships between `social` and `language`, which may not be fully captured by standard correlation measures.
...
## Model Comparison with bivariate Gaussian model in `CARBayes` package
...
The bivariate Bayesian hierarchical model with copula with the Clayton copula has the smallest WAIC value and the inverse of Gumbel copula has the smallest LOOIC value. Those two models both better fit the data than the bivariate Gaussian model (denoted as MVS).

## Visualization of the Results on Maps

We can visualize the fitted results as the model-based estimation on maps to see the spatial distribution of the response variables and the structured random effects comparing the direct estimates. The model-based estimation is the estimation of the vulnerability prevalence in the Social and Language domains, which is estimated by the inverse Gumbel copula (based on the criteria of LOOIC value). The direct estimation is estimated by the direct proportion of the vulnerable children to the total children in the areal units.
